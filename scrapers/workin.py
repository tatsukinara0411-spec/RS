...
class WorkinScraper(BaseScraper):
    site_name = "Workin"
    ...
    async def scrape_industry(self, context: BrowserContext, industry: str) -> list[Lead]:
        ...
        for page_no in range(1, 21):
            ...
            kw = quote(keyword)
            url = f"{BASE_URL}/search/results?s%5Bkeyword%5D={kw}&s%5Barea_ids%5D%5B%5D=13&page={page_no}"

            page = await context.new_page()
            try:
                await self.safe_goto(page, url)

                cards = await page.query_selector_all(".p-job-list__item")
                if not cards:
                    cards = await page.query_selector_all(".job-list__item")
                if not cards:
                ...
                if not cards:
                    logger.warning(f"[Workin] カードなし: {url}")
                    break

                logger.info(f"[Workin] {industry} p{page_no}: {len(cards)}件")
                ...
                logger.error(f"[Workin] エラー {url}: {e}")
                break
            finally:
                await page.close()
    ...
    async def _parse_card(self, card, industry: str, source_url: str) -> Lead | None:
        try:
            ...
            logger.debug(f"[Workin] カード解析エラー: {e}")
            return None
