...
BASE_URL = "https://job.jmedley.com"
...
class JmedleyScraper(BaseScraper):
    site_name = "ジョブメドレー"
    ...
    async def scrape_industry(self, context: BrowserContext, industry: str) -> list[Lead]:
        ...
        for page_no in range(1, 21):
            ...
            kw = quote(keyword)
            url = f"{BASE_URL}/search/?pref_ids%5B%5D=13&keyword={kw}&page={page_no}"

            page = await context.new_page()
            try:
                await self.safe_goto(page, url)

                cards = await page.query_selector_all(".c-job-list__item")
                if not cards:
                    cards = await page.query_selector_all(".job-item")
                if not cards:
                    cards = await page.query_selector_all("[class*='job-list'] > li")
                if not cards:
                    cards = await page.query_selector_all("article")

                if not cards:
                    logger.warning(f"[ジョブメドレー] カードなし: {url}")
                    break

                logger.info(f"[ジョブメドレー] {industry} p{page_no}: {len(cards)}件")
                ...
                logger.error(f"[ジョブメドレー] エラー {url}: {e}")
                break
            finally:
                await page.close()
    ...
    async def _parse_card(self, card, industry: str, source_url: str) -> Lead | None:
        try:
            ...
            company_name = ""
            corp_keywords = ["株式会社", "有限会社", "合同会社", "一般社団", "医療法人", "社会福祉", "学校法人"]
            for line in lines:
