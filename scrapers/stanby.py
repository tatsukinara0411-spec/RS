...
class StanbyScraper(BaseScraper):
    site_name = "スタンバイ"
    ...
    async def scrape_industry(self, context: BrowserContext, industry: str) -> list[Lead]:
        ...
        for page_no in range(1, 21):
            ...
            kw = quote(keyword)
            loc = quote(LOCATION)
            url = f"{BASE_URL}/search/?q={kw}&location={loc}&p={page_no}"
            ...
            try:
                ...
                if not cards:
                    cards = await page.query_selector_all(".result-item")
                if not cards:
                    cards = await page.query_selector_all("article")

                if not cards:
                    logger.warning(f"[スタンバイ] カードなし: {url}")
                    break

                logger.info(f"[スタンバイ] {industry} p{page_no}: {len(cards)}件")
                ...
                next_btn = await page.query_selector(f"a[href*='p={page_no + 1}']")
                if not next_btn:
                ...
                logger.error(f"[スタンバイ] エラー {url}: {e}")
                break
            finally:
                await page.close()
    ...
    async def _parse_card(self, card, industry: str, source_url: str) -> Lead | None:
        try:
            ...
            logger.debug(f"[スタンバイ] カード解析エラー: {e}")
            return None
